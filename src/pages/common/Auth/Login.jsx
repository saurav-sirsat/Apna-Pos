...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.log('Login fulfilled:', login.fulfilled.match(resultAction));
      ...
      if (login.fulfilled.match(resultAction)) {
        ...
        console.log('Login successful, letting App.jsx handle redirection');
      } else {
        console.log('Login failed, payload:', resultAction.payload);
        toast({
          title: "Error",
    ...
    try {
      const resultAction = await dispatch(forgotPassword(forgotEmail))
       if (forgotPassword.fulfilled.match(resultAction)) {
        ...
        console.log("error", resultAction.payload || error)
        toast({
        title: "Error",
          description: resultAction.payload || error || 'Failed to send reset email',
        variant: "destructive",
      })
      }
    } catch (error) {
          ...
          <div className="bg-card rounded-2xl shadow-xl p-8">
            ...
            <div className="mt-6 p-4 bg-gradient-to-r from-primary/10 to-emerald-500/10 rounded-xl border border-primary/20">
              <p className="text-sm text-muted-foreground text-center">
                <strong className="text-primary">Apna POS Demo Account:</strong><br />
                Email: demo@apnapos.com<br />
                Password: demo123
              </p>
            </div>
          </div>
